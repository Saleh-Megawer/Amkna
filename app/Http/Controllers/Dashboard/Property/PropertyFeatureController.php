...
class PropertyFeatureController extends Controller
{
    ...
    {
        $routes = [
            'index'   => route('properties.features.index'),
            'update'  => route('properties.features.update'),
            'store'   => route('properties.features.store'),
            'destroy' => route('properties.features.destroy'),
        ];
    ...
    public function index()
    {
        ...
        if ($id != 0) {
            ...
            $editRow    = [];
            $formOption = [
                'panelTitle'   => 'إضافة ميزة جديدة',
                'submitButton' => 'إضافة',
                'formAction'   => $this->routes('store'),
            ];
        }

        return view('dashboard.property.features.index', compact('rows', 'editRow', 'formOption', 'destroyRoute'));
    }
    ...
    public function store(Request $request)
    {
        ...
        if ($validator->fails()) {
            return Response::error('هذه الميزة مستخدمة مسبقاً.', ['style' => 'toastr']);
        }
        ...
        return Response::success('تم إضافة الميزة بنجاح', ['style' => 'toastr', 'reset' => true, 'reload' => true, 'time_out' => 2]);
    }
    ...
    public function update(Request $request)
    {
        ...
        // Return Message
        return Response::success('تم تحديث الميزة بنجاح', ['style' => 'toastr', 'redirect' => $this->routes('index'), 'reload' => true, 'time_out' => 2]);
    }
    ...
    public function destroy()
    {
        ...
        if (empty($row)) {
            ...
            $row->delete();
            //Message
            return Response::success('تم حذف الميزة بنجاح', ['style' => 'toastr']);
        }
    }
