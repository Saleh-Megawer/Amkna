@php
    $statusOptions = collect(App\Enums\OwnerAssociation\RequestStatus::cases())
        ->filter(fn($s) => $s->value !== 'pending')
        ->map(
            fn($s) => [
                'value' => $s->value,
                'label' => $s->label(),
                'color' => $s->color(),
            ],
        )
        ->values();
@endphp

{{-- Change Status Modal --}}
<div class="modal fade" id="changeStatusModal" tabindex="-1" role="dialog">
    <div class="modal-dialog modal-dialog-centered" role="document">
        <div class="modal-content border-0 shadow-lg">

            {{-- Header --}}
            <div class="modal-header bg-white border-bottom">
                <h5 class="modal-title font-weight-bold text-dark">
                    <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24"
                        fill="none" stroke="currentColor" stroke-width="2" class="ml-2">
                        <path d="M9 11l3 3l8 -8" />
                        <path d="M20 12v6a2 2 0 0 1 -2 2h-12a2 2 0 0 1 -2 -2v-12a2 2 0 0 1 2 -2h9" />
                    </svg>
                    تغيير حالة الطلب
                </h5>
                <button type="button" class="close" data-dismiss="modal">
                    <span>&times;</span>
                </button>
            </div>

            {{-- Body --}}
            <div class="modal-body">

                <form id="changeStatusForm">
                    @csrf

                    {{-- Current Request Info --}}
                    <div class="mb-4 p-3 bg-light rounded">
                        <h6 class="font-weight-bold mb-2 change-status-title"></h6>
                        <div class="d-flex align-items-center">
                            <small class="text-muted ml-2">الإجراء الحالي : </small>
                            <span class="badge badge-md font-11 change-status-current-badge"></span>
                        </div>
                    </div>

                    {{-- New Status --}}
                    <div class="form-group">
                        <label class="font-weight-600">اختر الحالة الجديدة <span class="text-danger">*</span></label>
                        <select name="status" class="form-control" required>
                            <option disabled value="">اختر الحالة</option>
                            @foreach ($statusOptions as $status)
                                <option value="{{ $status['value'] }}">{{ $status['label'] }}</option>
                            @endforeach
                        </select>
                    </div>

                    {{-- Admin Notes --}}
                    <div class="form-group">
                        <label class="font-weight-600">ملاحظات الإدارة <small
                                class="text-muted">(اختياري)</small></label>
                        <textarea name="admin_notes" class="form-control" rows="4" placeholder="أضف ملاحظات حول تغيير الحالة..."></textarea>
                        <small class="form-text text-muted">ستظهر هذه الملاحظات في تفاصيل الطلب</small>
                    </div>

                    <input type="hidden" name="request_id" id="change-status-request-id">

                </form>

            </div>

            {{-- Footer --}}
            <div class="modal-footer bg-white border-top">
                <button type="button" class="btn btn-main" id="submitChangeStatus">
                    <span class="btn-text">حفظ التغييرات</span>
                    <span class="btn-loading" style="display: none;">
                        <span class="spinner-border spinner-border-sm ml-2"></span>
                        جاري الحفظ...
                    </span>
                </button>
                <button type="button" class="btn btn-outline-main" data-dismiss="modal">إلغاء</button>
            </div>

        </div>
    </div>
</div>
